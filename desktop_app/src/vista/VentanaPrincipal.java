	...
	public VentanaPrincipal(Login login, Usuario user) {
		...
		if (user.getRol().getIdRol() == 3) {
			try {
				...
				if (puesto != null) {
					bienvenida += " (" + puesto.getCodigoPuesto() + ")";
				}
		...
		if (!esOperario) {
			...
			if (user.getRol().getIdRol() == 2) {
				chkSoloMisIncidencias.setBounds(520, 109, 150, 22);
		...
		btnAnadir.setBounds(esOperario ? 10 : 763, 104, 145, 32);
		...
		scrollPane.setBounds(10, esOperario ? 160 : 210, 1062, 387);
		...
		if (user.getRol().getIdRol() != 1 && user.getRol().getIdRol() != 2) {
			btnGestionar.setVisible(false);
		}
		contentPane.add(btnGestionar);

		// Jefe ST: botón exportar
		if (user.getRol().getIdRol() == 1) {
			...
			btnExportar.setBounds(763, 104, 145, 32);
