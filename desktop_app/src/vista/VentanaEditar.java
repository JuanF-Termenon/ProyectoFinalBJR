...
import com.st.repositories.NotaRepository;
...
public class VentanaEditar extends JFrame {
    ...
    public VentanaEditar(VentanaPrincipal ventana, DefaultTableModel modelo, int filaSeleccionada, Usuario user) {
        ...
        boolean esTecnicoOJefe = user.getRol().getIdRol() == 1 || user.getRol().getIdRol() == 2;
        ...
        JLabel lblNotasLabel = new JLabel("Notas internas:");
        lblNotasLabel.setFont(new Font("Tahoma", Font.BOLD, 12));
        lblNotasLabel.setBounds(20, 430, 120, 20);
        lblNotasLabel.setVisible(esTecnicoOJefe);
        contentPane.add(lblNotasLabel);

        txtNotas = new JTextArea();
        txtNotas.setEditable(false);
        txtNotas.setLineWrap(true);
        txtNotas.setWrapStyleWord(true);
        txtNotas.setBackground(new Color(250, 250, 230));
        txtNotas.setFont(new Font("Tahoma", Font.PLAIN, 11));

        StringBuilder sb = new StringBuilder();
        try {
            NotaRepository nRepo = new NotaRepository();
            ArrayList<Nota> notas = nRepo.findByIdIncidencia(idIncidencia);
            for (Nota n : notas) {
                sb.append("[").append(new SimpleDateFormat("dd/MM HH:mm").format(n.getFechaCreacion()))
                  .append("] ").append(n.getNombreUsuario()).append(":\n")
                  .append(n.getNota()).append("\n---\n");
            }
        } catch (SQLException ex) {
            sb.append("Error al cargar notas.");
        }
        txtNotas.setText(sb.toString());

        JScrollPane scrollNotas = new JScrollPane(txtNotas);
        scrollNotas.setBounds(20, 455, 500, 60);
        scrollNotas.setVisible(esTecnicoOJefe);
        contentPane.add(scrollNotas);

        JTextField inputNota = new JTextField();
        inputNota.setBounds(20, 525, 380, 30);
        inputNota.setVisible(esTecnicoOJefe);
        contentPane.add(inputNota);

        JButton btnAddNota = new JButton("A\u00F1adir");
        btnAddNota.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                String texto = inputNota.getText().trim();
                if (texto.isEmpty()) return;
                try {
                    NotaRepository nRepo = new NotaRepository(user.getIdUsuario());
                    nRepo.guardarNota(idIncidencia, user.getIdUsuario(), texto);
                    inputNota.setText("");
                    StringBuilder s = new StringBuilder();
                    try {
                        for (Nota n : nRepo.findByIdIncidencia(idIncidencia)) {
                            s.append("[").append(new SimpleDateFormat("dd/MM HH:mm").format(n.getFechaCreacion()))
                             .append("] ").append(n.getNombreUsuario()).append(":\n")
                             .append(n.getNota()).append("\n---\n");
                        }
                    } catch (SQLException ex) {}
                    txtNotas.setText(s.toString());
                } catch (SQLException ex) {
                    JOptionPane.showMessageDialog(null, "Error al guardar nota: " + ex.getMessage());
                }
            }
        });
        btnAddNota.setBackground(new Color(0, 102, 153));
        btnAddNota.setForeground(Color.WHITE);
        btnAddNota.setFont(new Font("Tahoma", Font.BOLD, 11));
        btnAddNota.setBorderPainted(false);
        btnAddNota.setFocusPainted(false);
        btnAddNota.setBounds(410, 525, 110, 30);
        btnAddNota.setVisible(esTecnicoOJefe);
        contentPane.add(btnAddNota);
        ...
        btnEliminar.setBounds(144, 580, 126, 36);
        btnEliminar.setVisible(user.getRol().getIdRol() == 1);
        ...
        btnGuardarCambios.setBounds(290, 580, 126, 36);
        ...
        btnAsignar.setBounds(10, 580, 126, 36);
        btnAsignar.setVisible(user.getRol().getIdRol() == 2);
        ...
        btnHistorial.setBounds(10, 580, 126, 36);
        btnHistorial.setVisible(user.getRol().getIdRol() == 1);
